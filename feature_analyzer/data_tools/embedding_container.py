...
from feature_analyzer.data_tools.feature_object import FeatureObject
from feature_analyzer.data_tools.attribute_table import AttributeTable
from feature_analyzer.query.csv_reader import CsvReader

from feature_analyzer.core.standard_fields import AttributeStandardFields as attr_field
from feature_analyzer.core.standard_fields import EmbeddingContainerStandardFields as container_fields

from feature_analyzer.utils.interpreter import CommandExecutor
...
class EmbeddingContainer(object):
    ...
    def __init__(self,
        ...
        assert landmark_size >= 0, 'landmark_size must larger than or equal to 0'
    ...
    def __repr__(self):
        ...
        if self._landmarks is not None:
            _content += 'landamrks: ({}, {})\n'.format(self.counts, self.landmark_size)
        _content += 'internals: '
    ...
    def _init_arrays(self, container_size, embedding_size, probability_size, landmark_size):
        ...
        if landmark_size != self._landmark_size:
            self._landmark_size = max(self._landmark_size, landmark_size)
        ...
        if self._landmark_size != 0:
            self._landmarks = np.empty((self._container_size,
                                        self._landmark_size), dtype=np.float32)
        self._current = 0

    # TODO: Reconsider signature of the unique interface
    # TODO: Add conf, landmark,
    def add(self, instance_id, label_id, embedding,
        ...
        if landmark is not None:
            assert landmark.shape[0] <= self._landmark_size, "Size of landmark vector is greater than the default."
        ...
        if landmark is not None:
            self._landmarks[self._current, ...] = landmark
    ...
    def landmarks(self):
        if self._landmarks is not None:
            return self._landmarks[:self._current]
        return np.array([])
    ...
    def reset(self,
        ...
        origin_landmark_size = self._landmark_size
        ...
        if landmark_size < 0:
            landmark_size = self._landmark_size
        elif landmark_size != origin_landmark_size:
            print('landmark size: {} -> {}'.format(origin_landmark_size, landmark_size))
    ...
    def save(self, path):
        ...
        if self.counts > 0 and self._landmarks is not None:
            feature_exporter.landmarks = self.landmarks
            print('Export landmarks with shape: {}'.format(self.landmarks.shape))
    ...
    def _from_np_array(self,
        ...
        if landmarks is not None:
            ...
            lm_num, landmark_size = landmarks.shape

            if lm_num != total_amount:
                raise ValueError('#of Instance does not match #of Landmarks, {} != {}'.format(
                    total_amount, lm_num))
    ...
    def _load_featobj(self, path):
        ...
        landmarks = feature_importer.landmarks
