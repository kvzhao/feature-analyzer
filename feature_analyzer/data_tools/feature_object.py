...
from feature_analyzer.core.standard_fields import FeatureObjectStandardFields as fields
...
class FeatureObjectBase(object):
    ...
    @property
    def landmarks(self):
        if self._array_name_map[fields.landmarks] is None:
            print('WARNING: Get the empty landmark array')
            return self._array_name_map[fields.landmarks]
        if len(self._array_name_map[fields.landmarks].shape) >= 3:
            print('NOTICE: Shape of given landmarks are {}, squeezed automatically.'.format(
                self._array_name_map[fields.landmarks].shape))
            _landmarks = np.squeeze(self._array_name_map[fields.landmarks])
        return self._array_name_map[fields.landmarks]

    @landmarks.setter
    def landmarks(self, _landmarks):
        self._check_numpy_arrlike(_landmarks)
        if len(_landmarks.shape) >= 3:
            print('NOTICE: Shape of given landmarks are {}, squeezed automatically.'.format(
                _landmarks.shape))
            _landmarks = np.squeeze(_landmarks)
        self._array_name_map[fields.landmarks] = _landmarks
